...
def show_tensor(tensor, resize_hw=None, top_k=50, mode='CHW', is_show=True,
    ...
    if not show_split:
        # sum可能会越界，所以需要归一化
        #tensor = np.abs(tensor[:, :, :channel // 2] - tensor[:, :, channel // 2:])
        # tensor1,tensor2 = np.expand_dims(np.sum(tensor[:, :, :channel // 2], axis=2),2),\
        #                   np.expand_dims(np.sum(tensor[:, :, channel // 2:], axis=2),2)
        # sum_tensor = np.max(np.concatenate([tensor1,tensor2],axis=2),axis=2)
        sum_tensor = np.max(tensor, axis=2)
