...
def get_criterion(opt):
    ...
    if opt.loss_function == 'contra_hybrid':
        criterion = contra_hybrid_loss().cuda()
...
def load_model(opt, device):
    ...
    if opt.model_name == 'diffFPN':
        model = LSNet_diffFPN().to(device)
    elif opt.model_name == 'thin_diffFPN':
        model = LSNet_thin_diffFPN().to(device)
    elif opt.model_name == 'denseFPN':
        model = LSNet_denseFPN().to(device)
