...
def full_path_loader(data_dir):
    ...
    for img in train_data:
        train_label_paths.append(data_dir + 'train/OUT_binary/' + img)
    for img in valid_data:
        val_label_paths.append(data_dir + 'val/OUT_binary/' + img)
...
def full_test_loader(data_dir):
    ...
    for img in test_data:
        test_label_paths.append(data_dir + 'test/OUT_binary/' + img)
...
def cdd_loader(img_path, label_path, aug):
    ...
    # img1 = cv2.imread(dir + 'A/' + name)
    # img2 = cv2.imread(dir + 'B/' + name)
    # label = cv2.imread(label_path.replace('.jpg','.png'),-1)
